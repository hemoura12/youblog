		...
		$.ajax({
			...
			success: function(obj){
				...
				for(var i in jsonObj.list){
 					if(i==0){
						outValues += "<div class = 'ul-table'><div class='ul-row'>";
						if(jsonObj.list[i].rowno==1){
							outValues += "<div class=\"session_name\"><span class=\"libray_title\">최신 게시글</span></div>";
						}
						outValues += "<div class='ul-table__tables'>";
					}
...
<!--4x4 배열 시작-->
    <div class="conBox" id="ctdiv">
        <div class = "ul-table">
            <div class="ul-row">
                <div class="session_name"><span class="libray_title">인기 게시글</span></div>
                <div class="ul-table__tables" id="favoritectdiv">
                <c:forEach items="${favoritelist}" var="fv">
                    <div class="table_1" onclick='location.href="detailpage?blogno=${fv.blogno}"'><img  src="resources/images/라이언 썸네일.png"/>
                        <label class ="blog_title">${fv.title}</label>
                        <label class ="blog_name">${fv.membername}</label>
                        <label class ="blog_view_time">조회수 ${fv.hits}·${fv.writedate}</label>
                    </div>
				</c:forEach>
                </div>
            </div>
